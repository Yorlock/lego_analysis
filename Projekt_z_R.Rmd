...
grouped_by_year_max_parts_gif <- animate(grouped_by_year_max_parts_plot, width = 480, height = 480)
...
ggplot(correlation_matrix_pearson_df, aes(Var1, Var2, fill = Freq)) +
...
ggplot(correlation_matrix_kendall_df, aes(Var1, Var2, fill = Freq)) +
  geom_tile(color = "purple") +
  scale_fill_distiller(palette = "Purples", direction = 1) +
...
ggplot(correlation_matrix_spearman_df, aes(Var1, Var2, fill = Freq)) +
  geom_tile(color = "purple") +
  scale_fill_distiller(palette = "Purples", direction = 1) +
...
## Top `r top_themes_1` tematów na podstawie ilości setów
```{r}
...
top_n_themes_plot <- ggplot(top_n_1_themes_data_df, aes(x=reorder(name_parent, Count_sets), y=Count_sets)) +
  ...
  coord_flip() +
  labs(title = paste0("Top ", top_themes_1, " najpopularniejszych tematów na \npodstawie ilości setów"), x = "Theme name", y = "Sum of sets") +
  theme_bw() +
  theme(legend.position = "none")
